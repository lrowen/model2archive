;This one for RADIO SHACK - LDOS 5.1 - 07/07/83
;       Change Log
;Log in drive if possible 04/26/83
;Store total head count for drive 04/29/83
;Padded to 399 bytes 06/10/83
;Added precomp reg init 07/07/83
;
...
*GET	MOD3/EQU
	ENDIF
*LIST	ON
;DVRWD/ASM - LDOS 5.1 - 01/13/83 - RS/CJ/LM
;*=*=*
;*=*=*
;       Calling Sequence
;        B  => disk command
...
;*****
	...
	LD	E,A		;Reset sector #
	INC	HL		;  and bump cyl #
REC1	LD	A,C		;Sector#/sectors per head
	CALL	DIVEA@
...
;*****
	...
	RET
HOWBIG	EQU	$-DISK
	DC	399-HOWBIG,0	;Pad out to 400 bytes
DISKEND	EQU	$
;*=*=*
...
HELLO$	DB	LF
	DB	'TRSHD - LDOS Winchester Disk '
	DB	'Driver - Version 5.1b',LF
	DB	'Copyright (C) 1983 by Logical Systems, '
