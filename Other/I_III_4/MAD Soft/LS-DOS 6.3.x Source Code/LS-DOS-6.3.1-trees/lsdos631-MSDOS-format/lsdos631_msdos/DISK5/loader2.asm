...
;
@WHERE	POP	HL		;get caller address
	JP	(HL)		;and return with PC
;
...
;*=*=*
	PUSH	AF		;pass flags to BC
	POP	BC		;BC = I/O result
	POP	AF		;A = old memory image
	CALL	SET_MOD		;restore previous memory
	POP	AF		;get last bank #
	LD	(LBANK$),A	;update
	PUSH	BC		;pass result back to AF
	POP	AF		;AF = result code
	POP	BC		;restore entry
	RET
GODOIO	JP	(IY)
...
SVCUSER	CP	26		;Check for @ERROR
	JR	Z,ERRSVC	; go if so
	LD	(LSVC$),A	;Store SVC request
	EX	(SP),HL		;PU  'RET' address
	LD	(SVCRET$),HL	; Store it down low
	EX	(SP),HL		;now put it back
ERRSVC	PUSH	HL		;Save HL
	ADD	A,A		;*2
	LD	H,SVCTAB$<-8	;P/u table entry
	LD	L,A		;Set up the low order
	CALL	$GETHL		;get table entry
...
EXOVR2	LD	B,0		;P/u ISAM # or zero
	LD	E,SFCB$&0FFH
	CALL	LOADER		;Read sys overlay
EXERR	POP	BC
	POP	DE
EXOVR3	LD	(TRANSFR+1),HL
	POP	HL
	RET	Z
	JR	SYSERR
;*=*=*
...
LDR07	CALL	LDR15		;Bypass 2nd X'02'
	JP	GETADR		;P/u transfer address
;*****
;	grab load block
;*****
...
;*=*=*
MOD3BUF	EQU	4300H		;mod III loader buffer
@IPL	DI			;Interrupts off
	LD	A,1		;bit 0
	OUT	(0F9H),A	;enable rom
	RST	0		;Go to boot ROM
;*****
