; RECUT - Recut utility for updating master, perserve serial #'s
;
...
NOTIN	LD	HL,NOTIN$
	DB	0DDH
BADSRC	LD	HL,BADSRC$	;"Source not MASTER
	DB	0DDH
BADDST	LD	HL,BADDST$	;"Destination not MASTER
	@@LOGOT
	LD	HL,-1
	LD	SP,(SAVSTK)
	JR	EXIT2
...
;
SIGNON$	DB	'RECUT'
	DB	' - 6.3.0 - MASTER update program Version 1.0.0',LF,LF,CR
;
...
BADSRC$	DB	'SOURCE not a MASTER disk',CR
BADDST$	DB	'DESTINATION not a MASTER disk',CR
...
DONE$	DB	LF,LF,'Disk RECUT complete, RECUT another (Y/N) ? ',LF,CR
...
RESTRT:	CALL	MOUNT		;Have disks mounted
	...
	LD	(DE),A
	LD	A,(SRCDRV)	;P/U src drive #
	LD	C,A		;put in C
	@@RSTOR		; RESTORE THE DRIVE
	LD	A,(DSTDRV)	;P/U dst drive #
	LD	C,A		;put in C
	@@RSTOR		; RESTORE THE DRIVE
;
;	Check that both are MASTERS and P/U SERIAL and ID numbers
;
	LD	A,(SRCDRV)	;check the source first
	LD	C,A
	LD	D,SYS0TRK
	LD	E,SYS0SEC
	LD	HL,TEMPBUF
	@@RDSEC
	JP	NZ,SRCERR	;Leave if error
	LD	L,SYS0OFF
	DEC	HL
	DEC	HL		;point to # in serial
	LD	A,'#'
	CP	(HL)		;check it
	JP	NZ,BADSRC	;if found # .. continue
;	check destination and P/U serial and ID #s
	LD	A,(DSTDRV)	;get drive #
	LD	C,A
	LD	D,SYS0TRK
	LD	E,SYS0SEC
	LD	HL,TEMPBUF
	@@RDSEC			;get the serial sector
	JP	NZ,DSTERR
	LD	L,SYS0OFF	;set the offset
	DEC	HL
	DEC	HL
	LD	A,'#'		;Check for MASTER type
	CP 	(HL)
	JP	NZ,BADDST	;abort if not
	INC 	HL
	INC	HL
	LD	DE,SERBUF	;set up to store serial #
	LD	BC,10
	LDIR			;got serial number
	LD	A,(DSTDRV)	;get the drive
	LD	C,A
	LD	D,SYS3TRK
	LD	E,SYS3SEC
	LD	HL,TEMPBUF
	...
	INC	HL
	LD	A,(HL)
	LD	(IDBUF+1),A	;got the ID #
	LD	A,(SRCDRV)	;restore both drives
	LD	C,A
	@@RSTOR
	LD	A,(DSTDRV)
	LD	C,A
...
VER2:	LD	A,(IY+6)	;See if at end
	...
	JP	MAINLP
;
ALLDUN:	LD	A,(DSTDRV)	;DUPE complete, restore serial and ID
	LD	C,A
	LD	D,SYS0TRK
	LD	E,SYS0SEC
	LD	HL,TEMPBUF
	@@RDSEC			;get the SERIAL # sector
	JP	NZ,DSTERR
	LD	L,SYS0OFF	;point to serial #
	LD	DE,SERBUF
	EX	DE,HL		;setup to move serial
	LD	BC,10
	LDIR
	LD	HL,TEMPBUF	;put it back
	LD	A,(DSTDRV)
	LD	C,A
	LD	D,SYS0TRK
	LD	E,SYS0SEC
	@@WRSEC			;to the destination
	JP	NZ,DSTERR
	LD	A,(DSTDRV)	;now restore ID #
	LD	C,A
	LD	D,SYS3TRK
	LD	E,SYS3SEC
	@@RDSEC			;P/U ID sector
...
BUFF	EQU	$
;
ENDLOC	EQU	$+6506		;For free mem check
	END	BEGIN
